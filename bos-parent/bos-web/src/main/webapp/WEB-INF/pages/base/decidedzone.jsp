...
<script type="text/javascript">
	...
	function doDelete(){
		...
		if(rows.length == 0){
			//没有选中记录，弹出提示
			$.messager.alert("提示信息","请选择需要删除的改定区！","warning");
		}else{
			//选中了取派员,弹出确认框
			$.messager.confirm("删除确认","删除该定区之前请先确认是否解绑分区？",function(r){
				if(r){
					var array = new Array();
					//确定,发送请求
					//获取所有选中的取派员的id
	...
	function doClickRow(index,data){
	...
	function doDblClickRow(rowIndex, rowData){
		...
		$("#updateDecidedzoneWindow").form("load",rowData);
	    var did = $("#did").val();
	    console.log(did);
	    $('#subareaGridupdate').datagrid( {
	    	fitColumns:true,
	    	singleSelect:false,
			rownumbers : true,
			striped : true,
			url : 'subareaAction_listajaxall.action?did='+did
...
<body class="easyui-layout" style="visibility:hidden;">
			...
			<form id="addDecidedzoneForm" method="post"
				...
				<table class="table-edit" width="80%" align="center">
					...
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:100px" 
					...
					<tr>
					   <td><span style="color:red">选择的分区不能为空</span></td>
					</tr>
				</table>
			</form>
		</div>
	...
	<!-- 修改定区 -->
	<div class="easyui-window" title="定区修改" id="updateDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				...
				<script type="text/javascript">
					$(function(){
						$("#update").click(function(){
							var v = $("#updateDecidedzoneWindow").form("validate");
							if(v){
								
								$("#updateDecidedzone").submit();
							}
						});
					});
				</script>
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="updateDecidedzone" method="post"
				action="decidedzoneAction_update.action">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
						<td colspan="2">定区信息</td>
					</tr>
					<tr>
						<td>定区编码</td>
						<td><input type="text" name="id" id="did" class="easyui-validatebox" required="true"/></td>
					</tr>
					<tr>
						<td>定区名称</td>
						<td><input type="text" name="name" class="easyui-validatebox" required="true"/></td>
					...
					<tr>
						<td>选择取派员</td>
						<td>
							<input required="true" class="easyui-combobox" name="staff.id"  
    							data-options="valueField:'id',textField:'name',
					...
					<tr height="300">
						<td valign="top">关联分区</td>
						<td>
							<table id="subareaGridupdate" class="easyui-datagrid" border="false" style="width:300px;height:100px" >
								<thead>  
							        <tr>  
							            <th data-options="field:'subareaid',width:30,checkbox:true">编号</th>  
							            <th data-options="field:'addresskey',width:150">关键字</th>  
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#associationBtn").click(function(){
									    ...
									    var coun=$("#associationSelect").val();
									    if(coun ==null) {
									    	$.messager.alert("提示信息","关联的客户不能为空！","warning");
									    }else{
									    	$("#customerForm").submit();
									    } 
									});
